function [f,p,uncertainty_f, uncertainty_p,kvg,iter,corp,covp,covr,stdresid,Z,r2] = GIOleasqr(x,y,q,pin,F,stol,niter,wt,dp,dFdp,options)

%% Original header
% function [f,p,kvg,iter,corp,covp,covr,stdresid,Z,r2] = GIOleasqr(x,y,q,pin,F,stol,niter,wt,dp,dFdp,options)
...
if ~exist('verbose','var'), %If verbose undefined, print nothing
...
if nargin <= 7
    wt=ones(length(y),1); % SMB modification
end	
if nargin <= 6
    niter=1000;
end
...
m=length(y); n=length(pin); p=pin;[m1,~]=size(x);
...
%
f = feval(F,x,q,p); fbest = f; pbest = p;

if Debug == 1
    figure(1000), plot(x,f);
end
...
for iter=1:niter
    if Debug == 1
        fprintf(' go on with iteration %d (of %d)?',iter,niter);
        input('?')
    end
    
    pprev=pbest;
    prt = feval(dFdp,x,fbest,pprev,q,dp,F);
    if Debug == 1
        figure(1001), plot(x,prt);
        fprintf('derivative done\n');
    end
    r=wt.*(y-fbest);
    sprev=sbest;
    sgoal=(1-stol)*sprev;
...
p=pbest;
% f: computed optimum function
f=fbest;
% ss: scalar sum of squares
ss=sbest; %#ok<NASGU>
...
jac=feval(dFdp,x,f,p,q,dp,F);

if Debug == 1
    figure(1003), plot(x,jac);
end

msk = dp ~= 0; % vector containing number of elements different than  zero;
n = sum(msk);           % reduce n to equal number of estimated parameters
jac = jac(:, msk);	% limit jacobian to fitted parameters
...
if vernum(1) >= 4, % Sparse matrices defined in version 4;
...
covp = jtgjinv * jac'*Qinv * Vy * Qinv * jac * jtgjinv; %#ok<MINV> % Eq. 7-5-13, Bard %cov of parm est
...
%Estimate parameter errors: Author Giorgio Signorello (gio@zurich.ibm.com)
[uncertainty_f, uncertainty_p] = ParametersError(jac,resid,p,confidence);
...
function [uncertainty_f, uncertainty_p] = ParametersError(jac,resid,p,confidence)
% The function uses the Jacobian of the fitting function (jac), 
% calculated with the optimal parameters (p), as well as the residuals (resid)
% to return the:
% 
% - the confidence interval for the estimated value of the function (uncertainty_f),
%   and 
% 
% - the marginal confidence intervals for the parameters (uncertainty_p),
%   i.e. the uncertainty on the estimated parameters;
%
% the uncertainties are calculated with the level of confidence
% (confidence) given by the user;
%
% The formulas are exctrated from the following reference:
%  Nonlinear regression analysis and its applications
%  Douglas M. Bates, Donald G. Watts
%  http://books.google.ch/books?id=m5I_AQAAIAAJ&num=18&source=gbs_navlinks_s
% 

[~,Erre]=qr(jac,0);

degrees_of_freedom = length(resid)-length(p);

S = sqrt((resid'*resid)./degrees_of_freedom);

% finds the percentile of the T student, or Fisher distribution, with N-P, or P and N-P degrees
% of freedom, searching for the zero of the difference between the
% cumulative density function and the confidence wanted;

percentile_p = abs(tinv( (100-confidence)/200,degrees_of_freedom));

percentile_f = finv(confidence/100,length(p),degrees_of_freedom);

inversErre = inv(Erre);

uncertainty_p = S * percentile_p * sqrt(diag(inversErre*inversErre',0));

vt_invR = jac*inversErre;

uncertainty_f = S * sqrt(length(p)*percentile_f) * sqrt(diag(vt_invR*vt_invR',0));
