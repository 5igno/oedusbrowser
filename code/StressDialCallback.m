function handles = StressDialCallback( hObject, handles )
...
if isnumeric(id_candidate) && ~isempty(id_candidate)
    if 1 <= id_candidate  && id_candidate <= handles.data.n_stress
        handles.data.choose.id_stress = ceil(id_candidate);
    end
end
